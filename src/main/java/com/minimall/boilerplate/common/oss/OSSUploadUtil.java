...
import com.minimall.boilerplate.common.StringHelper;
...
public class OSSUploadUtil {
  ...
  public static String uploadFileMu(MultipartFile file, String fileType){
    config = config==null?new OSSConfig():config;
    String fileEcodeName =file.getOriginalFilename();
    try {
      fileEcodeName = new String(file.getOriginalFilename().getBytes("ISO-8859-1"), "UTF-8");
      fileEcodeName= StringHelper.chinaToUnicode(fileEcodeName);
    } catch (UnsupportedEncodingException e) {
      e.printStackTrace();
    }

    //后缀名
    String hostLastName=fileEcodeName.split("//.")[fileEcodeName.split("//.").length-1];
    String fileName = config.getPicLocation()+UUID.randomUUID().toString().toUpperCase().replace("-", "")+"."+hostLastName ; //文件名，根据UUID来
    return putObjectMuFile(file,fileType,fileName);
  }

  public static String uploadFileMu(byte[] bytes){
    config = config==null?new OSSConfig():config;

    String fileName = config.getPicLocation()+UUID.randomUUID().toString().toUpperCase().replace("-", "")+".png" ; //文件名，根据UUID来

    return putObjectMuFile(new ByteArrayInputStream(bytes),"",fileName);
  }
  ...
  private static String putObjectMuFile(MultipartFile file, String fileType, String fileName){
    config = config==null?new OSSConfig():config;
    String url = null;      //默认null
    OSSClient ossClient = null;
    try {
      ...
      meta.setContentType(OSSUploadUtil.contentType(fileType));       // 设置上传内容类型// 被下载时网页的缓存行为
      PutObjectRequest request = new PutObjectRequest(config.getBucketName(), fileName,input,meta);           //创建上传请求
      ossClient.putObject(request);
      url = config.getEndpoint().replaceFirst("http://","http://"+config.getBucketName()+".")+"/"+fileName;       //上传成功再返回的文件路径
    } catch (OSSException oe) {
  ...
  private static String putObjectMuFile(InputStream file, String fileType, String fileName){
    config = config==null?new OSSConfig():config;
    String url = null;      //默认null
    OSSClient ossClient = null;
    try {
      ...
      ossClient.putObject(config.getBucketName(),fileName,input);
      url = config.getEndpoint().replaceFirst("http://","http://"+config.getBucketName()+".")+"/"+fileName;       //上传成功再返回的文件路径
    } catch (OSSException oe) {
      oe.printStackTrace();
      return null;
